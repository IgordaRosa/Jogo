...
bool Renderer::Initialize(Window * window, Graphics * graphics)
{
    ...
    // descreve o input layout dos v�rtices
    ...
    // gera �ndices para o n�mero m�ximo de sprites suportados
    ...
    // calcula a matriz de transforma��o
...
void Renderer::RenderBatch(ID3D11ShaderResourceView * texture, SpriteData ** sprites, uint cont)
{
    ...
    while (cont > 0)
    {
        ...
        // quantidade de sprite � maior do que o espa�o dispon�vel
        if (batchSize > remainingSpace)
        {
            // se o tamanho dispon�vel � muito pequeno
            if (remainingSpace < MinBatchSize)
            {
                // volte ao �nicio do buffer
                vertexBufferPosition = 0;
                batchSize = (cont < MaxBatchSize) ? cont : MaxBatchSize;
            }
            else
            {
                // restrinja a quantidade de sprites pelo espa�o sobrando
        ...
        // gera posi��es dos v�rtices de cada sprite que ser� desenhado nesse lote
        ...
        {        
            ...
            // carrega informa��es do sprite em registros SIMD
            ...
            // altera a escala do offset de origem pelo tamanho da fonte, tomando cuidado para evitar overflow se a regi�o fonte for zero
            ...
            // converte a regi�o fonte de texels para o formato de coordenadas de textura mod-1
            origin *= inverseTextureSize;

            // se o tamanho de destino � relativo a regi�o fonte, converte-o para pixels
            destinationSize *= textureSize;

            // calcula uma matriz de rota��o 2x2
            ...
            // os quatro v�rtices do sprite s�o calculados a partir de transforma��es dessas posi��es unit�rias
            ...
            for (int i = 0; i < VerticesPerSprite; ++i)
            {
                ...
                // Escreve posi��o como um Float4, mesmo sendo VertexPositionColor::position um XMFLOAT3.
                // Isso � mais r�pido e inofensivo porque estamos apenas invalidando o primeiro elemento
                // do campo cor, que ser� imediatamente sobrescrito com seu valor correto.
        ...
        // avan�a a posi��o no vertex buffer
        vertexBufferPosition += batchSize;

        // avan�a a posi��o no vetor de sprites
        sprites += batchSize;
